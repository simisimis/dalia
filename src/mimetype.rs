...
/// MimeTypeError enumerates all errors returned by mimetype module.
...
pub fn image_or_video<P: AsRef<Path>>(path: &P) -> Result<bool, MimeTypeError> {
    if fs::metadata(path)?.is_dir() {
        ...
        match infer::get_from_path(path) {
            ...
            Ok(_) => Ok(false),
            Err(e) => Err(MimeTypeError::IOError(e)),
        }
    }
}
